...
class XPYKeyTests: XCTestCase {
    func testRFC6979() {
        let verifyRFC6979: (String, AnyObject, String, String) -> Void = { keyhex, msg, khex, sighex in
            let hash: NSData
            
            if let msgString = msg as? String {
                hash = XPYSHA256(msgString.dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false))
            }
            else {
                hash = msg as! NSData
            }
            
            let key = XPYKey(privateKey: XPYDataFromHex(keyhex))
            let k = key.signatureNonceForHash(hash)
            XCTAssertEqual(XPYDataFromHex(khex), k, "Must produce matching k nonce.")
            let sig = key.signatureForHash(hash)
            XCTAssertEqual(XPYDataFromHex(sighex), sig, "Must produce matching signature.")
        }
        ...
        verifyRFC6979("22a47fa09a223f2aa079edf85a7c2d4f8720ee63e502ee2869afab7de234b80c",
            XPYHash256("test message".dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false)),
    ...
    func testDiffieHellman() {
        let alice = XPYKey(privateKey: XPYDataFromHex("c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa73bd4e39a8a"))
        alice.publicKeyCompressed = true
        
        let bob = XPYKey(privateKey: XPYDataFromHex("2db963f0fe106f483d9afa73bd4e39a8ac4bbcb1fbec99d65bf59d85c8cb62ee"))
        ...
        XCTAssertEqual(dh1.publicKey, XPYDataFromHex("03735932754bc16e10febe40ee0280906d29459d477442f1838dcf27de3b5d9699"), "")
        XCTAssertEqual(dh2.publicKey, XPYDataFromHex("03735932754bc16e10febe40ee0280906d29459d477442f1838dcf27de3b5d9699"), "")
    }
    ...
    func testCanonicality() {
        ...
        let errors = data.reduce(Array<NSError>()) { acc, datum in
            var error: NSError?
            if !XPYKey.isCanonicalSignatureWithHashType(datum, verifyLowerS: true, error: &error) {
                return acc + [error!]
            }
            return acc
        }
    ...
    func testRandomKeys() {
        ...
        for i in 0 ..< 32 {
            let k = XPYKey()
            //println("key = \(XPYHexFromData(k.privateKey))")
    ...
    func testBasicSigning() {
        ...
        let messageData = message.dataUsingEncoding(NSUTF8StringEncoding, allowLossyConversion: false)!
        let secret = XPYDataFromHex("c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa73bd4e39a8a")
...
//        println("key.publicKey = \(key.publicKey)")
        ...
        let pubkeyAddress = XPYPublicKeyAddress(data: key.publicKey.XPYHash160())
        let privkeyAddress = XPYPrivateKeyAddress(data: key.privateKey)
...
//        println("Valid \(key.isValidSignature(signature, hash: messageData.SHA256()))")
        ...
        let key2 = XPYKey(publicKey: pubkey)
...
//            println("Signature: \(signature.hex()) (\(signature.length) bytes)")
            let key2 = XPYKey.verifySignature(signature, forMessage: message)
...
//            println("Valid: \(key.isValidSignature(signature, forMessage: message))")
        ...
        encapsulate {
            let signature = XPYDataFromHex("1B158259BD8EEB198BABBCC4308CDFB8E8068F0A712CAC634257933A072EA6DB7" + "BEB3308F4C937D4F397A2A782BF12884045C27430719A2890F0127B4732D9CF0D")
            
            let key = XPYKey.verifySignature(signature, forMessage: "Test message")
