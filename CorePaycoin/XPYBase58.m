...
static const char* XPYBase58Alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

NSMutableData* XPYDataFromBase58(NSString* string)
{
    return XPYDataFromBase58CString([string cStringUsingEncoding:NSASCIIStringEncoding]);
}

NSMutableData* XPYDataFromBase58Check(NSString* string)
{
    return XPYDataFromBase58CheckCString([string cStringUsingEncoding:NSASCIIStringEncoding]);
}

NSMutableData* XPYDataFromBase58CString(const char* cstring)
{
    ...
    {
        const char* p1 = strchr(XPYBase58Alphabet, *p);
        if (p1 == NULL)
        {
            while (isspace(*p))
                p++;
        ...
        BN_set_word(&bnChar, (BN_ULONG)(p1 - XPYBase58Alphabet));
    ...
    int nLeadingZeros = 0;
    for (const char* p = cstring; *p == XPYBase58Alphabet[0]; p++)
        nLeadingZeros++;
...
NSMutableData* XPYDataFromBase58CheckCString(const char* cstring)
{
    if (cstring == NULL) return nil;
    
    NSMutableData* result = XPYDataFromBase58CString(cstring);
    size_t length = result.length;
    if (length < 4)
    {
        return nil;
    }
    NSData* hash = XPYHash256([result subdataWithRange:NSMakeRange(0, length - 4)]);
...
char* XPYBase58CStringWithData(NSData* data)
{
    if (!data) return NULL;
    ...
    NSMutableData* tmp = XPYReversedMutableData(data);
    tmp.length += 1;
    ...
    while (BN_cmp(&bn, &bn0) > 0)
    {
        ...
        [stringData appendBytes:XPYBase58Alphabet + c length:1];
    }
    finish();
    ...
    {
        [stringData appendBytes:XPYBase58Alphabet + 0 length:1];
    }
    
    // Convert little endian std::string to big endian
    XPYDataReverse(stringData);
    
    [stringData appendBytes:"" length:1];
    
    char* r = malloc(stringData.length);
    memcpy(r, stringData.bytes, stringData.length);
    XPYDataClear(stringData);
    return r;
}

// String in Base58 with checksum
char* XPYBase58CheckCStringWithData(NSData* immutabledata)
{
    if (!immutabledata) return NULL;
    // add 4-byte hash check to the end
    NSMutableData* data = [immutabledata mutableCopy];
    NSData* checksum = XPYHash256(data);
    [data appendBytes:checksum.bytes length:4];
    char* result = XPYBase58CStringWithData(data);
    XPYDataClear(data);
    return result;
}

NSString* XPYBase58StringWithData(NSData* data)
{
    if (!data) return nil;
    char* s = XPYBase58CStringWithData(data);
    id r = [NSString stringWithCString:s encoding:NSASCIIStringEncoding];
    XPYSecureClearCString(s);
    free(s);
    return r;
}


NSString* XPYBase58CheckStringWithData(NSData* data)
{
    if (!data) return nil;
    char* s = XPYBase58CheckCStringWithData(data);
    id r = [NSString stringWithCString:s encoding:NSASCIIStringEncoding];
    XPYSecureClearCString(s);
    free(s);
    return r;
