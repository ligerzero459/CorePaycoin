...
#define XPYBigNumberCompare(a, b) (BN_cmp(&(a->_bignum), &(b->_bignum)))

@implementation XPYBigNumber {
    @package
    BIGNUM _bignum;
...
- (void) throwIfImmutable {
    if (_immutable) {
        @throw [NSException exceptionWithName:@"Immutable XPYBigNumber is modified" reason:@"" userInfo:nil];
    }
}
...
- (XPYBigNumber*) copyWithZone:(NSZone *)zone
{
    XPYBigNumber* to = [[XPYBigNumber alloc] init];
...
- (XPYMutableBigNumber*) mutableCopyWithZone:(NSZone *)zone
{
    XPYMutableBigNumber* to = [[XPYMutableBigNumber alloc] init];
    if (BN_copy(&(to->_bignum), &_bignum))
    {
        return to;
    }
...
- (BOOL) isEqual:(XPYBigNumber*)other
{
    if (![other isKindOfClass:[XPYBigNumber class]]) return NO;
    return XPYBigNumberCompare(self, other) == NSOrderedSame;
}

- (NSComparisonResult)compare:(XPYBigNumber *)other
{
    return XPYBigNumberCompare(self, other);
}
...
{
    return (XPYBigNumberCompare(self, other) <= 0) ? self : other;
}
...
- (BOOL) less:(XPYBigNumber *)other           { return XPYBigNumberCompare(self, other) <  0; }
- (BOOL) lessOrEqual:(XPYBigNumber *)other    { return XPYBigNumberCompare(self, other) <= 0; }
- (BOOL) greater:(XPYBigNumber *)other        { return XPYBigNumberCompare(self, other) >  0; }
- (BOOL) greaterOrEqual:(XPYBigNumber *)other { return XPYBigNumberCompare(self, other) >= 0; }
...
- (NSString*) stringInBase:(NSUInteger)base
{
    ...
    else
    {
        while (BN_cmp(&bn, &bn0) > 0)
        {
            ...
            {
                NSLog(@"XPYBigNumber: stringInBase failed to BN_div");
                break;
            }
            BN_copy(&bn, &dv);
...
//
// Paycoin only uses this "compact" format for encoding difficulty
...
{
    BN_CTX* pctx = BN_CTX_new();
    XPYBigNumber* r = [XPYBigNumber new];
    XPYBigNumber* m = [XPYBigNumber new];
...
@implementation XPYMutableBigNumber
...
+ (instancetype) zero
{
    return [[XPYBigNumber zero] mutableCopy];
}

+ (instancetype) one
{
    return [[XPYBigNumber one] mutableCopy];
}

+ (instancetype) negativeOne
{
    return [[XPYBigNumber negativeOne] mutableCopy];
}
...
- (instancetype) rshift:(unsigned int)shift // >>=
{
    ...
    XPYMutableBigNumber* a = [XPYMutableBigNumber one];
    [a lshift:shift];
