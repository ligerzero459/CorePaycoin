...
typedef NS_ENUM(NSInteger, XPYScriptSimulationOptions) {
    // Default is strict conservative simulation (uncompressed pubkeys, no P2SH interpretation)
    XPYScriptSimulationDefault = 0,

    // Use compressed pubkeys instead of uncompressed ones.
    XPYScriptSimulationCompressedPublicKeys = 1 << 0,

    // Instead of failing on P2SH, assume it is 2-of-3 multisig (the most popular one).
    XPYScriptSimulationMultisigP2SH = 1 << 1,
};

@class XPYAddress;
@class XPYScriptHashAddress;
@class XPYScriptHashAddressTestnet;
@interface XPYScriptChunk : NSObject
...
@property(nonatomic, readonly) XPYOpcode opcode;
...
@interface XPYScript : NSObject<NSCopying>
...
// List of parsed chunks of the script (XPYScriptChunk)
...
// This is the most popular type that is used to pay to "addresses" (e.g. 1CXPYGivXmHQ8ZqdPgeMfcpQNJrqTrSAcG).
...
// Returns YES if the script consists of push data operations only (including OP_<N>). Aka isPushOnly in PaycoinQT.
// Used in BIP16 (P2SH).
...
// Returns XPYPublicKeyAddress or XPYScriptHashAddress if the script is a standard output script for these addresses.
// If the script is something different, returns nil.
@property(nonatomic, readonly) XPYAddress* standardAddress;

// Wraps the recipient into an output P2SH script (OP_HASH160 <20-byte hash of the recipient> OP_EQUAL).
@property(nonatomic, readonly) XPYScript* scriptHashScript;
...
@property(nonatomic, readonly) XPYScriptHashAddress* scriptHashAddress;
@property(nonatomic, readonly) XPYScriptHashAddressTestnet* scriptHashAddressTestnet;
...
// DEPRECATION WARNING: These methods were moved to XPYKey class.
