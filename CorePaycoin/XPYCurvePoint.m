...
+ (XPYBigNumber*) curveOrder
{
    static XPYBigNumber* order;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        order = [[XPYBigNumber alloc] initWithUnsignedBigEndian:XPYDataWithHexCString("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141")];
    });
    return order;
}
...
- (id) initEmpty
{
    if (self = [super init])
    {
        ...
        if (!_group)
        {
            NSLog(@"XPYCurvePoint: EC_GROUP_new_by_curve_name(NID_secp256k1) failed");
            goto finish;
        }
        ...
        if (!_point)
        {
            NSLog(@"XPYCurvePoint: EC_POINT_new(_group) failed");
            goto finish;
        }
        ...
        if (!_bnctx)
        {
            NSLog(@"XPYCurvePoint: BN_CTX_new() failed");
            goto finish;
        }
...
- (instancetype) multiply:(XPYBigNumber*)number
{
...
- (instancetype) add:(XPYCurvePoint*)otherPoint
{
...
- (instancetype) addGeneratorMultipliedBy:(XPYBigNumber*)number
{
    if (!number) return nil;
...
- (XPYBigNumber*) x
{
    ...
    XPYBigNumber* result = [[XPYBigNumber alloc] initWithBIGNUM:bn];
    BN_CTX_end(_bnctx);
    return result;
}
...
{
    return [[XPYCurvePoint alloc] initWithEC_POINT:_point];
}

- (BOOL) isEqual:(XPYCurvePoint*)otherPoint
{
...
{
    return [NSString stringWithFormat:@"<XPYCurvePoint:0x%p %@>", self, XPYHexFromData(self.data)];
}
