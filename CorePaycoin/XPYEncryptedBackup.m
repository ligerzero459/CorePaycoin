...
#import "XPYEncryptedBackup.h"
#import "XPYData.h"
#import "XPYBase58.h"
#import "XPYKey.h"
#import "XPYNetwork.h"
#import "XPYMerkleTree.h"
#import "XPYProtocolSerialization.h"
#import <CommonCrypto/CommonCrypto.h>

@interface XPYEncryptedBackup ()
@property(nonatomic, readwrite) XPYEncryptedBackupVersion version;
...
@implementation XPYEncryptedBackup

- (id) initWithBackupKey:(NSData*)backupKey {
    if (!backupKey) return nil;
    if (self = [super init]) {
        self.version = XPYEncryptedBackupVersion1;
...
+ (instancetype) encrypt:(NSData*)data backupKey:(NSData*)backupKey timestamp:(NSTimeInterval)timestamp {
    XPYEncryptedBackup* b = [[XPYEncryptedBackup alloc] initWithBackupKey:backupKey];
...
+ (NSData*) backupKeyForNetwork:(XPYNetwork*)network masterKey:(NSData*)masterKey {
    if (!network || network.isMainnet) {
        return XPYHMACSHA256(masterKey, [@"Automatic Backup Key Mainnet" dataUsingEncoding:NSASCIIStringEncoding]);
    } else {
        return XPYHMACSHA256(masterKey, [@"Automatic Backup Key Testnet" dataUsingEncoding:NSASCIIStringEncoding]);
    }
}

+ (XPYKey*) authenticationKeyWithBackupKey:(NSData*)backupKey {
    XPYKey* key = [[XPYKey alloc] initWithPrivateKey:XPYHMACSHA256(backupKey, [@"Authentication Key" dataUsingEncoding:NSUTF8StringEncoding])];
...
+ (NSString*) walletIDWithAuthenticationKey:(NSData*)authPubkey {
    ...
    [data appendData:XPYHash160(authPubkey)];
    return XPYBase58CheckStringWithData(data);
}


- (NSData*) encryptionKey {
    return [XPYHMACSHA256(self.backupKey, [@"Encryption Key" dataUsingEncoding:NSUTF8StringEncoding]) subdataWithRange:NSMakeRange(0, 16)];
}
...
- (NSData*) encrypt {
    ...
    XPYKey* ak = [self authenticationKey];
    ...
    [result appendData:[XPYProtocolSerialization dataForVarString:ciphertext]];
    [result appendData:[XPYProtocolSerialization dataForVarString:signature]];
    return result;
}
...
- (NSData*) decrypt {
    ...
    NSUInteger ctlen = 0;
    NSData* ciphertext = [XPYProtocolSerialization readVarStringFromData:[payload subdataWithRange:NSMakeRange(fixedOffset, payload.length - fixedOffset)] readBytes:&ctlen];

    if (!ciphertext) {
        return nil;
    }

    NSData* signature = [XPYProtocolSerialization readVarStringFromData:
    ...
    if (![ak isValidSignature:signature hash:XPYHash256(sigData)]) {
        return nil;
    }
...
- (NSData*) ivForPlaintext:(NSData*)plaintext ek:(NSData*)ek {
    return [XPYHMACSHA256(ek, plaintext) subdataWithRange:NSMakeRange(0, 16)];
}
...
- (NSData*) merkleRootForCiphertext:(NSData*)data {
    ...
    */
    ...
    XPYMerkleTree* mt = [[XPYMerkleTree alloc] initWithDataItems:dataItems];
    return mt.merkleRoot;
}

- (NSData*) dataForSigning:(XPYEncryptedBackupVersion)version timestamp:(NSTimeInterval)timestamp iv:(NSData*)iv merkleRoot:(NSData*)merkleRoot {
