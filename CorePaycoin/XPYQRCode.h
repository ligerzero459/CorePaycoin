...
@interface XPYQRCode : NSObject

#if TARGET_OS_IPHONE
/*!
 * Returns a QR code image with a given size.
 * The `string` is typically a paycoin address in Base58Check format.
 */
+ (UIImage*) imageForString:(NSString*)string size:(CGSize)size scale:(CGFloat)scale;

/*!
 * Returns a QR code image with a given size.
 * The `url` is typically a paycoin payment request "paycoin:<address>?amount=..."
 */
