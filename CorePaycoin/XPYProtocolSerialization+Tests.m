...
@implementation XPYProtocolSerialization (Tests)

+ (void) assertNumber:(uint64_t)number serializesToHex:(NSString*)hexForm
{
    NSUInteger requiredLength = hexForm.length / 2;
    
    NSData* data = [XPYProtocolSerialization dataForVarInt:number];
    //NSLog(@"data = %@", data);
    NSAssert([data isEqualToData:XPYDataFromHex(hexForm)], @"Should encode correctly");
    uint64_t value = 0;
    NSUInteger len = [XPYProtocolSerialization readVarInt:&value fromData:data];
    ...
    [stream open];
    len = [XPYProtocolSerialization readVarInt:&value fromStream:stream];
    [stream close];
