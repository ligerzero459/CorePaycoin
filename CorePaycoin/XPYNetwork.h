...
@class XPYBigNumber;
@class XPYBlock;
@interface XPYNetwork : NSObject <NSCopying>

- (id) initWithName:(NSString*)name;

// Available networks

// Main Paycoin network, singleton instance.
+ (XPYNetwork*) mainnet;
...
@property(nonatomic) XPYBigNumber* proofOfWorkLimit;

// Array of pairs @[ @(int <height>), NSData* <hash> ] sorted by height.
@property(nonatomic) NSArray* checkpoints;


// Returns a checkpoint hash if it exists or XPY256Zero if there is no checkpoint at such height.
