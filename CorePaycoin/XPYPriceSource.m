...
#import "XPYPriceSource.h"
#import "XPYErrors.h"



@implementation XPYPriceSourceResult
@end

@implementation XPYPriceSource
...
- (NSString*) name {
    [NSException raise:@"XPYPriceSource method not implemented" format:@"You must override -name method."];
    return nil;
}

// Supported currency codes ("USD", "EUR", "CNY" etc).
- (NSArray*) currencyCodes {
    [NSException raise:@"XPYPriceSource method not implemented" format:@"You must override -currencyCodes method."];
    return nil;
}

// Returns a NSURLRequest to fetch the avg price.
- (NSURLRequest*) requestForCurrency:(NSString*)currencyCode {
    [NSException raise:@"XPYPriceSource method not implemented" format:@"You must override either -loadPriceForCurrency:error: or -requestForCurrency:"];
    return nil;
}
...
- (XPYPriceSourceResult*) resultFromParsedData:(id)parsedData currencyCode:(NSString*)currencyCode error:(NSError**)errorOut {
    [NSException raise:@"XPYPriceSource method not implemented" format:@"You must override -resultFromParsedData:currencyCode:error: method."];
    return nil;
}
...
- (void) loadPriceForCurrency:(NSString*)currencyCode
            completionHandler:(void(^)(XPYPriceSourceResult* result, NSError* error))completionBlock {
...
- (void) loadPriceForCurrency:(NSString*)currencyCode
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        NSError* error = nil;
        XPYPriceSourceResult* result = [self loadPriceForCurrency:currencyCode error:&error];
...
- (XPYPriceSourceResult*) loadPriceForCurrency:(NSString*)currencyCode error:(NSError**)errorOut {
    ...
    if (![self.currencyCodes containsObject:currencyCode]) {
        NSError* error = [NSError errorWithDomain:XPYErrorDomain
                                             code:XPYErrorUnsupportedCurrencyCode
                                         userInfo:@{}];
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if (httpResponse.statusCode < 200 || httpResponse.statusCode >= 300) {
            error = [NSError errorWithDomain:XPYErrorDomain
    ...
    XPYPriceSourceResult* result = [self resultFromParsedData:parsedData currencyCode:currencyCode error:&error];
...
@implementation XPYPriceSourceCoindesk
...
- (XPYPriceSourceResult*) resultFromParsedData:(id)parsedData currencyCode:(NSString*)currencyCode error:(NSError**)errorOut {
    ...
    XPYPriceSourceResult* result = [[XPYPriceSourceResult alloc] init];
...
@implementation XPYPriceSourceWinkdex
...
@implementation XPYPriceSourceCoinbase
...
@implementation XPYPriceSourcePaymium
