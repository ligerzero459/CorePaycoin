...
@interface XPYScriptMachine ()

// Constants
@property(nonatomic) NSData* blobFalse;
@property(nonatomic) NSData* blobZero;
@property(nonatomic) NSData* blobTrue;
@property(nonatomic) XPYBigNumber* bigNumberZero;
@property(nonatomic) XPYBigNumber* bigNumberOne;
@property(nonatomic) XPYBigNumber* bigNumberFalse;
@property(nonatomic) XPYBigNumber* bigNumberTrue;
@end

// We try to match PaycoinQT code as close as possible to avoid subtle incompatibilities.
...
@implementation XPYScriptMachine {
...
- (id) init
{
    if (self = [super init])
    {
        ...
        _bigNumberZero = [[XPYBigNumber alloc] initWithInt32:0];
        _bigNumberOne = [[XPYBigNumber alloc] initWithInt32:1];
...
- (id) initWithTransaction:(XPYTransaction*)tx inputIndex:(uint32_t)inputIndex
{
    if (!tx) return nil;
    // PaycoinQT would crash right before VerifyScript if the input index was out of bounds.
...
{
    XPYScriptMachine* sm = [[XPYScriptMachine alloc] init];
...
{
    return (_blockTimestamp >= XPY_BIP16_TIMESTAMP);
}

- (BOOL) verifyWithOutputScript:(XPYScript*)outputScript error:(NSError**)errorOut
{
    // self.inputScript allows to override transaction so we can simply testing.
    XPYScript* inputScript = self.inputScript;
    ...
    if (!inputScript)
    {
        ...
        {
            [NSException raise:@"XPYScriptMachineException"  format:@"transaction and valid inputIndex are required for script verification."];
            return NO;
        }
        if (!outputScript)
        {
            [NSException raise:@"XPYScriptMachineException"  format:@"non-nil outputScript is required for script verification."];
            return NO;
        }

        XPYTransactionInput* txInput = self.transaction.inputs[self.inputIndex];
    ...
    if (shouldVerifyP2SH)
    {
        // PaycoinQT: scriptSig must be literals-only
        ...
        if (stackForP2SH.count == 0)
        {
            ...
            [NSException raise:@"XPYScriptMachineException"  format:@"internal inconsistency: stackForP2SH cannot be empty at this point."];
            return NO;
        }
        
        // Instantiate the script from the last data on the stack.
        XPYScript* providedScript = [[XPYScript alloc] initWithData:[stackForP2SH lastObject]];
...
- (BOOL) runScript:(XPYScript*)script error:(NSError**)errorOut
{
    if (!script)
    {
        [NSException raise:@"XPYScriptMachineException"  format:@"non-nil script is required for -runScript:error: method."];
        return NO;
    }
    
    if (script.data.length > XPY_MAX_SCRIPT_SIZE)
    {
    ...
    [script enumerateOperations:^(NSUInteger opIndex, XPYOpcode opcode, NSData *pushdata, BOOL *stop) {
...
- (BOOL) executeOpcodeError:(NSError**)errorOut
{
    ...
    XPYOpcode opcode = _opcode;
    NSData* pushdata = _pushdata;
    
    if (pushdata.length > XPY_MAX_SCRIPT_ELEMENT_SIZE)
    {
        if (errorOut) *errorOut = [self scriptError:NSLocalizedString(@"Pushdata chunk size is too big.", @"")];
        return NO;
    }
    
    if (opcode > OP_16 && !_pushdata && ++_opCount > XPY_MAX_OPS_PER_SCRIPT)
    {
    ...
    else if (shouldExecute || (OP_IF <= opcode && opcode <= OP_ENDIF))
    {
        switch (opcode)
        {
            ...
            {
                // ( -- value)
                XPYBigNumber* bn = [[XPYBigNumber alloc] initWithInt64:(int)opcode - (int)(OP_1 - 1)];
            ...
            case OP_FROMALTSTACK:
            {
                ...
                {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"%@ requires one item on altstack", @""), XPYNameForOpcode(opcode)]];
                    return NO;
                }
            ...
            {
                // -- stacksize
                XPYBigNumber* bn = [[XPYBigNumber alloc] initWithInt64:_stack.count];
                [_stack addObject:bn.signedLittleEndian];
            }
            break;
            ...
            case OP_ROLL:
            {
                ...
                XPYBigNumber* bn = [self bigNumberAtIndex:-1];
                ...
                {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of items for %@: %d.", @""), XPYNameForOpcode(opcode), n]];
                    return NO;
                }
            ...
            case OP_SIZE:
            {
                ...
                XPYBigNumber* bn = [[XPYBigNumber alloc] initWithUInt64:[self dataAtIndex:-1].length];
                [_stack addObject:bn.signedLittleEndian];
            }
            break;
            ...
            case OP_0NOTEQUAL:
            {
                ...
                XPYMutableBigNumber* bn = [self bigNumberAtIndex:-1];
                ...
                switch (opcode)
                {
                    ...
                    case OP_NEGATE:     [bn multiply:[XPYBigNumber negativeOne]]; break;
                    case OP_ABS:        if ([bn less:_bigNumberZero]) [bn multiply:[XPYBigNumber negativeOne]]; break;
            ...
            case OP_MAX:
            {
                ...
                XPYMutableBigNumber* bn1 = [self bigNumberAtIndex:-2];
                XPYMutableBigNumber* bn2 = [self bigNumberAtIndex:-1];
                ...
                switch (opcode)
                {
                    ...
                    case OP_BOOLAND:             bn = [[XPYMutableBigNumber alloc] initWithInt32:![bn1 isEqual:_bigNumberZero] && ![bn2 isEqual:_bigNumberZero]]; break;
                    case OP_BOOLOR:              bn = [[XPYMutableBigNumber alloc] initWithInt32:![bn1 isEqual:_bigNumberZero] || ![bn2 isEqual:_bigNumberZero]]; break;
                    case OP_NUMEQUAL:            bn = [[XPYMutableBigNumber alloc] initWithInt32: [bn1 isEqual:bn2]]; break;
                    case OP_NUMEQUALVERIFY:      bn = [[XPYMutableBigNumber alloc] initWithInt32: [bn1 isEqual:bn2]]; break;
                    case OP_NUMNOTEQUAL:         bn = [[XPYMutableBigNumber alloc] initWithInt32:![bn1 isEqual:bn2]]; break;
                    case OP_LESSTHAN:            bn = [[XPYMutableBigNumber alloc] initWithInt32:[bn1 less:bn2]]; break;
                    case OP_GREATERTHAN:         bn = [[XPYMutableBigNumber alloc] initWithInt32:[bn1 greater:bn2]]; break;
                    case OP_LESSTHANOREQUAL:     bn = [[XPYMutableBigNumber alloc] initWithInt32:[bn1 lessOrEqual:bn2]]; break;
                    case OP_GREATERTHANOREQUAL:  bn = [[XPYMutableBigNumber alloc] initWithInt32:[bn1 greaterOrEqual:bn2]]; break;
            ...
            case OP_WITHIN:
            {
                ...
                XPYMutableBigNumber* bn1 = [self bigNumberAtIndex:-3];
                XPYMutableBigNumber* bn2 = [self bigNumberAtIndex:-2];
                XPYMutableBigNumber* bn3 = [self bigNumberAtIndex:-1];
            ...
            case OP_HASH256:
            {
                ...
                {
                    hash = XPYHash160(data);
                }
                else if (opcode == OP_HASH256)
                {
                    hash = XPYHash256(data);
                }
                [self popFromStack];
            ...
            case OP_CODESEPARATOR:
            {
                ...
                // Paycoind will extract subscript *including* this codeseparator. But all codeseparators will be stripped out eventually
                // when we compute a hash of transaction. Just to keep ourselves close to paycoind for extra asfety, we'll do the same here.
            ...
            case OP_CHECKSIGVERIFY:
            {
                ...
                XPYScript* subscript = [_script subScriptFromIndex:_lastCodeSeparatorIndex];

                // Drop the signature, since there's no way for a signature to sign itself.
                // Normally we neither have signatures in the output scripts, nor checksig ops in the input scripts.
                // In early days of Paycoin (before July 2010) input and output scripts were concatenated and executed as one,
                ...
                // See https://paycointalk.org/index.php?topic=278992.0 for more info.
                [subscript deleteOccurrencesOfData:signature];

                NSError* sigerror = nil;
                BOOL failed = NO;
                if (_verificationFlags & XPYScriptVerificationStrictEncoding)
                {
                    if (![XPYKey isCanonicalPublicKey:pubkeyData error:&sigerror])
                    {
                        failed = YES;
                    }
                    if (!failed && ![XPYKey isCanonicalSignatureWithHashType:signature
                                                                 verifyLowerS:!!(_verificationFlags & XPYScriptVerificationEvenS)
                                                                       error:&sigerror])
                    {
                        failed = YES;
                    }
            ...
            case OP_CHECKMULTISIGVERIFY:
            {
                ...
                XPYBigNumber* bn = [self bigNumberAtIndex:-i];
                ...
                if (keysCount < 0 || keysCount > XPY_MAX_KEYS_FOR_CHECKMULTISIG)
                {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of keys for %@: %d.", @""), XPYNameForOpcode(opcode), keysCount]];
                    return NO;
                }
                
                _opCount += keysCount;
                
                if (_opCount > XPY_MAX_OPS_PER_SCRIPT)
                {
                ...
                XPYBigNumber* bn2 = [self bigNumberAtIndex:-i];
                ...
                {
                    if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Invalid number of signatures for %@: %d.", @""), XPYNameForOpcode(opcode), keysCount]];
                    return NO;
                }
                ...
                while (success && sigsCount > 0)
                {
                    ...
                    if (_verificationFlags & XPYScriptVerificationStrictEncoding)
                    {
                        ...
                        if (validMatch && ![XPYKey isCanonicalSignatureWithHashType:signature
                                                                        verifyLowerS:!!(_verificationFlags & XPYScriptVerificationEvenS)
                                                                              error:&sigerror])
                        {
                            validMatch = NO;
            ...
            default:
                if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Unknown opcode %d (%@).", @""), opcode, XPYNameForOpcode(opcode)]];
                return NO;
        }
    }
...
- (BOOL) checkSignature:(NSData*)signature publicKey:(NSData*)pubkeyData subscript:(XPYScript*)subscript error:(NSError**)errorOut
{
    XPYKey* pubkey = [[XPYKey alloc] initWithPublicKey:pubkeyData];
    
    if (!pubkey)
    {
        if (errorOut) *errorOut = [self scriptError:[NSString stringWithFormat:NSLocalizedString(@"Public key is not valid: %@.", @""),
                                                     XPYHexFromData(pubkeyData)]];
        return NO;
    }
    ...
    XPYSignatureHashType hashType = ((unsigned char*)signature.bytes)[signature.length - 1];
    ...
    //NSLog(@"XPYScriptMachine: Hash for input %d [%d]: %@", _inputIndex, hashType, XPYHexFromData(sighash));
...
{
    return [NSError errorWithDomain:XPYErrorDomain
...
- (NSError*) scriptErrorOpcodeRequiresItemsOnStack:(NSUInteger)items
{
    if (items == 1)
    {
        return [NSError errorWithDomain:XPYErrorDomain
                                   code:XPYErrorScriptError
                               userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %d item on stack.", @""), XPYNameForOpcode(_opcode), items]}];
    }
    return [NSError errorWithDomain:XPYErrorDomain code:XPYErrorScriptError userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:NSLocalizedString(@"%@ requires %d items on stack.", @""), XPYNameForOpcode(_opcode), items]}];
}
...
#define XPYNormalizeIndex(list, i) (i < 0 ? (list.count + i) : i)

- (NSData*) dataAtIndex:(NSInteger)index
{
    return _stack[XPYNormalizeIndex(_stack, index)];
}

- (void) swapDataAtIndex:(NSInteger)index1 withIndex:(NSInteger)index2
{
    [_stack exchangeObjectAtIndex:XPYNormalizeIndex(_stack, index1)
                withObjectAtIndex:XPYNormalizeIndex(_stack, index2)];
}

// Returns bignum from pushdata or nil.
- (XPYMutableBigNumber*) bigNumberAtIndex:(NSInteger)index
{
    NSData* data = [self dataAtIndex:index];
    if (!data) return nil;
    
    // PaycoinQT throws "CastToBigNum() : overflow" and then catches it inside EvalScript to return false.
    ...
    // Get rid of extra leading zeros like PaycoinQT does:
    ...
    XPYMutableBigNumber* bn = [[XPYMutableBigNumber alloc] initWithSignedLittleEndian:[[XPYBigNumber alloc] initWithSignedLittleEndian:data].signedLittleEndian];
    return bn;
}
...
{
    [_stack removeObjectAtIndex:XPYNormalizeIndex(_stack, index)];
}

// -1 means last item
- (void) popFromStack
{
    [_stack removeObjectAtIndex:XPYNormalizeIndex(_stack, -1)];
}
