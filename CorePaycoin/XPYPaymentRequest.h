...
extern NSInteger const XPYPaymentRequestVersion1;
extern NSInteger const XPYPaymentRequestVersionOpenAssets1;

extern NSString* __nonnull const XPYPaymentRequestPKITypeNone;
extern NSString* __nonnull const XPYPaymentRequestPKITypeX509SHA1;
extern NSString* __nonnull const XPYPaymentRequestPKITypeX509SHA256;

// Special value indicating that amount on the output is not specified.
extern XPYAmount const XPYUnspecifiedPaymentAmount;

// Status allows to correctly display information about security of the request to the user.
typedef NS_ENUM(NSInteger, XPYPaymentRequestStatus) {
    // Payment request is valid and the user can trust it.
    XPYPaymentRequestStatusValid                 = 0, // signed with a valid and known certificate.

    XPYPaymentRequestStatusNotCompatible         = 100, // version is not supported (currently only v1 is supported)

    // These allow Payment Request to be accepted with a warning to the user.
    XPYPaymentRequestStatusUnsigned              = 101, // PKI type is "none"
    XPYPaymentRequestStatusUnknown               = 102, // PKI type is unknown (for forward compatibility may allow sending or warn to upgrade).

    // These generally mean we should decline the Payment Request.
    XPYPaymentRequestStatusExpired               = 201,
    XPYPaymentRequestStatusInvalidSignature      = 202,
    XPYPaymentRequestStatusMissingCertificate    = 203,
    XPYPaymentRequestStatusUntrustedCertificate  = 204,
};

@class XPYNetwork;
@class XPYPayment;
@class XPYPaymentACK;
@class XPYPaymentRequest;
@class XPYPaymentDetails;
...
// }
@interface XPYPaymentRequest : NSObject

// Version of the payment request and payment details.
// Default is XPYPaymentRequestVersion1.
...
// See XPYPaymentRequestPKIType* constants.
...
// A XPYPaymentDetails object.
@property(nonatomic, readonly, nonnull) XPYPaymentDetails* details;
...
@property(nonatomic, readonly) XPYPaymentRequestStatus status;
...
- (nullable XPYPayment*) paymentWithTransaction:(nullable XPYTransaction*)tx;

- (nullable XPYPayment*) paymentWithTransactions:(nullable  NSArray*)txs memo:(nullable NSString*)memo;

@end

@interface XPYPaymentDetails : NSObject
...
// Unspecified amounts are set to XPY_MAX_MONEY so you can know if zero amount was actually specified (e.g. for OP_RETURN or proof-of-burn etc).
@property(nonatomic, readonly, nonnull) NSArray* /*[XPYTransactionOutput]*/ outputs;
...
@property(nonatomic, readonly, nonnull) NSArray* /*[XPYTransactionInput]*/ inputs;
...
// One or more valid, signed Paycoin transactions that fully pay the PaymentRequest
@property(nonatomic, readonly, nonnull) NSArray* /*[XPYTransaction]*/ transactions;

// Output scripts and amounts. Amounts are optional and can be zero.
@property(nonatomic, readonly, nonnull) NSArray* /*[XPYTransactionOutput]*/ refundOutputs;
...
// it is sent from the merchant's server to the paycoin wallet in response to a Payment message.
@interface XPYPaymentACK : NSObject
...
@property(nonatomic, readonly, nonnull) XPYPayment* payment;

// Note that should be displayed to the customer giving the status of the transaction
// (e.g. "Payment of 1 XPY for eleven tribbles accepted for processing.")
@property(nonatomic, readonly, nullable) NSString* memo;
