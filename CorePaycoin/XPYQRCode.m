...
#if TARGET_OS_IPHONE
@interface XPYQRCodeScannerView : UIView <AVCaptureMetadataOutputObjectsDelegate>
...
{
    return [[XPYQRCodeScannerView alloc] initWithDetectionBlock:detectionBlock];
}
#endif
...
#if TARGET_OS_IPHONE
@implementation XPYQRCodeScannerView

- (id) initWithDetectionBlock:(void(^)(NSString* message))detection
{
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds])
    {
        self.sessionQueue = dispatch_queue_create("XPYQRCodeScannerView", NULL);
...
- (void) prepareScanner
{
    ...
    if (!input)
    {
        NSLog(@"XPYQRCodeScannerView: Failed to instantiate a video device: %@", [error localizedDescription]);
        return;
    }
    ...
    {
        NSLog(@"XPYQRCodeScannerView: Failed to lock device for configuration: %@", [error localizedDescription]);
    }
    ...
    {
        NSLog(@"XPYQRCodeScannerView: QRCode not found in availableMetadataObjectTypes: %@", output.availableMetadataObjectTypes);
        [self cleanup];
        return;
    }
