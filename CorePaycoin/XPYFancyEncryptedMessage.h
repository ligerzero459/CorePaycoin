...
// Messages are encrypted with recipient's public key (Paycoin-compatible, that is secp256k1)
...
// 1 byte      proof-of-work difficulty target (similar to "bits" field in XPYBlockHeader). SHA512^2 of the entire encrypted message should be below target.
...
// R bits      recipient's address (XPYHash160(pubkey)), from 0 to 20 bytes. Only first R bits are used.
...
// 1,2,3,5,9 bytes of message length (M) as 'CompactSize' variable-length integer encoding. See XPYProtocolSerialization.
...
static unsigned char XPYFancyEncryptedMessageVersion[4] = {0xc1, 0xb9, 0xf0, 0xe3};

typedef NS_ENUM(uint8_t, XPYFancyEncryptedMessageAddressLength) {
    // No address is given, recipient must attempt to decrypt the message to figure if he is indeed a recipient.
    XPYFancyEncryptedMessageAddressLengthNone           = 0,
    
    // First 4 bits of the address. To route message efficiently to 6% of all users yet keeping recipient secret.
    XPYFancyEncryptedMessageAddressLengthLightRouting   = 4,
    ...
    XPYFancyEncryptedMessageAddressLengthNormalRouting  = 7,
    
    // Fingerprint is a short 32-bit identifier to be compact, but yet allow fast identification like in BIP32 / XPYKeychain.
    XPYFancyEncryptedMessageAddressLengthFingerprint    = 32,
    
    // Full 160-bit address fully identifying the recipient.
    XPYFancyEncryptedMessageAddressLengthFull           = 160,
};

@class XPYKey;
@interface XPYFancyEncryptedMessage : NSObject
...
- (NSData*) encryptedDataWithKey:(XPYKey*)recipientKey seed:(NSData*)seed;
