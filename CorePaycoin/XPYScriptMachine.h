...
typedef NS_ENUM(NSUInteger, XPYScriptVerification) {
    XPYScriptVerificationStrictEncoding = (1U << 1), // enforce strict conformance to DER and SEC2 for signatures and pubkeys (aka SCRIPT_VERIFY_STRICTENC)
    XPYScriptVerificationEvenS          = (1U << 2), // enforce lower S values (below curve halforder) in signatures (aka SCRIPT_VERIFY_EVEN_S, depends on STRICTENC)
};
...
@interface XPYScriptMachine : NSObject <NSCopying>

// "To" transaction that is signed by an inputScript.
// Required parameter.
@property(nonatomic) XPYTransaction* transaction;
...
@property(nonatomic) XPYScript* inputScript;
...
// One can be stricter to not relay transactions with non-canonical signatures and pubkey (as PaycoinQT does).
// Defaults in CorePaycoin: be liberal in what you accept and conservative in what you send.
// So we try to create canonical purist transactions but have no problem accepting and working with non-canonical ones.
@property(nonatomic) XPYScriptVerification verificationFlags;
