...
/*!
 * Class to compose and handle various Paycoin URLs according to BIP21.
 * See: https://github.com/paycoin/bips/blob/master/bip-0021.mediawiki
 */
@class XPYAddress;
@class XPYAssetID;
@interface XPYPaycoinURL : NSObject

/*!
 * Encoded address.
 */
@property(nonatomic, nullable) XPYAddress* address;
 ...
 */
@property(nonatomic, nullable) XPYAssetID* assetID;
...
/*!
 * Returns YES if it is a pure paycoin URL. That does not specify asset_id and not uses openassets: scheme.
 */
@property(nonatomic, readonly) BOOL isValidPaycoinURL;
...
/*!
 * Makes a URL in form "paycoin:<address>?amount=1.2345&label=<label>.
 * @param address Address to be rendered in base58 format.
 * @param amount  Amount in satoshis. Note that URI scheme dictates to render this amount as a decimal number in XPY.
 * @param label   Optional label.
 */
+ (nonnull NSURL*) URLWithAddress:(nonnull XPYAddress*)address amount:(XPYAmount)amount label:(nullable NSString*)label;

/*!
 * Instantiates if URL is a valid paycoin: URL.
...
/*!
 * Instantiates an empty Paycoin URL.
