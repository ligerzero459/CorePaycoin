...
#define CHECK_IF_CLEARED if (_cleared) { [[NSException exceptionWithName:@"XPYKey: instance was already cleared." reason:@"" userInfo:nil] raise]; }

#define XPYCompressedPubkeyLength   (33)
#define XPYUncompressedPubkeyLength (65)

static BOOL    XPYKeyCheckPrivateKeyRange(const unsigned char *secret, size_t length);
static BOOL    XPYKeyCheckSignatureElement(const unsigned char *bytes, int length, BOOL half);
static int     XPYRegenerateKey(EC_KEY *eckey, BIGNUM *priv_key);
static NSData* XPYSignatureHashForBinaryMessage(NSData* data);
static int     ECDSA_SIG_recover_key_GFp(EC_KEY *eckey, ECDSA_SIG *ecsig, const unsigned char *msg, int msglen, int recid, int check);

@interface XPYKey ()
@end

@implementation XPYKey {
    BOOL _cleared;
    EC_KEY* _key;
...
{
    XPYPrivateKeyAddress* addr = [XPYPrivateKeyAddress addressWithString:wifString];
    if (![addr isKindOfClass:[XPYPrivateKeyAddress class]])
    {
        return nil;
    }
...
- (NSData*)signatureForHash:(NSData*)hash withHashType:(XPYSignatureHashType)hashType
{
    return [self signatureForHash:hash appendHashType:YES hashType:hashType];
}

- (NSData*)signatureForHash:(NSData*)hash appendHashType:(BOOL)appendHashType hashType:(XPYSignatureHashType)hashType
{
    CHECK_IF_CLEARED;
    ...
    XPYMutableBigNumber* privkeyBN = [[XPYMutableBigNumber alloc] initWithBIGNUM:privkeyBIGNUM];
    XPYBigNumber* n = [XPYCurvePoint curveOrder];

    NSMutableData* kdata = [self signatureNonceForHash:hash];
    XPYMutableBigNumber* k = [[XPYMutableBigNumber alloc] initWithUnsignedBigEndian:kdata];
    [k mod:n]; // make sure k belongs to [0, n - 1]

    XPYDataClear(kdata);

    XPYCurvePoint* K = [[XPYCurvePoint generator] multiply:k];
    XPYBigNumber* Kx = K.x;

    XPYBigNumber* hashBN = [[XPYBigNumber alloc] initWithUnsignedBigEndian:hash];

    // Compute s = (k^-1)*(h + Kx*privkey)

    XPYBigNumber* signatureBN = [[[privkeyBN multiply:Kx mod:n] add:hashBN mod:n] multiply:[k inverseMod:n] mod:n];
...
- (NSMutableData*) signatureNonceForHash:(NSData*)hash {
    ...
    XPYMutableBigNumber* hashModOrder = [[[XPYMutableBigNumber alloc] initWithUnsignedBigEndian:hash] mod:order];
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        XPYBigNumber* bn = [[XPYBigNumber alloc] initWithUnsignedBigEndian:[NSData dataWithBytesNoCopy:t length:sizeof(t) freeWhenDone:NO]];
...
- (XPYCurvePoint*) curvePoint
{
    CHECK_IF_CLEARED;
    const EC_POINT* ecpoint = EC_KEY_get0_public_key(_key);
    XPYCurvePoint* cp = [[XPYCurvePoint alloc] initWithEC_POINT:ecpoint];
    return cp;
}
...
- (void) setPublicKey:(NSData *)publicKey
{
    ...
    _publicKeyCompressed = ([self lengthOfPubKey:_publicKey] == XPYCompressedPubkeyLength);
...
- (void) setDERPrivateKey:(NSData *)DERPrivateKey
{
    ...
    XPYDataClear(_publicKey); _publicKey = nil;
    [self prepareKeyIfNeeded];
...
- (void) generateKeyPair
{
    ...
    do {
        RAND_bytes(bytes, 32);
    } while (!XPYKeyCheckPrivateKeyRange(bytes, 32));
...
{
    CHECK_IF_CLEARED;
    XPYKey* newKey = [[XPYKey alloc] initWithNewKeyPair:NO];
    if (_key) newKey->_key = EC_KEY_dup(_key);
    return newKey;
}

- (BOOL) isEqual:(XPYKey*)otherKey
{
    CHECK_IF_CLEARED;
...
{
    return [NSString stringWithFormat:@"<XPYKey:0x%p %@>", self, XPYHexFromData(self.publicKeyCached)];
}

- (NSString*) debugDescription
{
    return [NSString stringWithFormat:@"<XPYKey:0x%p pubkey:%@ privkey:%@>", self, XPYHexFromData(self.publicKeyCached), XPYHexFromData(self.privateKey)];
}
...
- (NSUInteger) lengthOfPubKey:(NSData*)data
{
    ...
    if (header == 2 || header == 3)
        return XPYCompressedPubkeyLength;
    if (header == 4 || header == 6 || header == 7)
        return XPYUncompressedPubkeyLength;
    return 0;
}
...
#pragma mark - XPYAddress Import/Export
...
- (XPYPublicKeyAddress*) publicKeyAddress
{
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [XPYPublicKeyAddress addressWithData:XPYHash160(pubkey)];
}

- (XPYPublicKeyAddress*) address
{
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
...
- (XPYPublicKeyAddressTestnet*) addressTestnet
{
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [XPYPublicKeyAddressTestnet addressWithData:XPYHash160(pubkey)];
}

- (XPYPublicKeyAddress*) compressedPublicKeyAddress
{
    CHECK_IF_CLEARED;
...
- (XPYPublicKeyAddress*) uncompressedPublicKeyAddress
{
    CHECK_IF_CLEARED;
...
- (XPYPrivateKeyAddress*) privateKeyAddress
{
    CHECK_IF_CLEARED;
    NSMutableData* privkey = self.privateKey;
    if (privkey.length == 0) return nil;
    
    XPYPrivateKeyAddress* result = [XPYPrivateKeyAddress addressWithData:privkey publicKeyCompressed:self.isPublicKeyCompressed];
    XPYDataClear(privkey);
    return result;
}


- (XPYPrivateKeyAddressTestnet*) privateKeyAddressTestnet
{
    CHECK_IF_CLEARED;
    NSMutableData* privkey = self.privateKey;
    if (privkey.length == 0) return nil;

    XPYPrivateKeyAddressTestnet* result = [XPYPrivateKeyAddressTestnet addressWithData:privkey publicKeyCompressed:self.isPublicKeyCompressed];
    XPYDataClear(privkey);
    return result;
}
...
- (NSData*) compactSignatureForHash:(NSData*)hash
{
    ...
    if (nBitsR <= 256 && nBitsS <= 256)
    {
        ...
        for (int i=0; i < 4; i++)
        {
            ...
            XPYKey* key2 = [[XPYKey alloc] initWithNewKeyPair:NO];
...
+ (XPYKey*) verifyCompactSignature:(NSData*)compactSignature forHash:(NSData*)hash
{
    ...
    // It will be updated via direct access to _key ivar.
    XPYKey* key = [[XPYKey alloc] initWithNewKeyPair:NO];
...
{
    CHECK_IF_CLEARED;
    XPYKey* key = [[self class] verifyCompactSignature:signature forHash:hash];
...
- (XPYKey*) diffieHellmanWithPrivateKey:(XPYKey*)privkey {
    ...
    XPYCurvePoint* curvePoint = self.curvePoint;
    XPYBigNumber* pk = [[XPYBigNumber alloc] initWithUnsignedBigEndian:privkey.privateKey];
    ...
    XPYKey* dhKey = [[XPYKey alloc] initWithCurvePoint:curvePoint];
...
#pragma mark - Paycoin Signed Message





// Returns a signature for a message prepended with "Paycoin Signed Message:\n" line.
...
{
    if (!data) return nil;
    return [self compactSignatureForHash:XPYSignatureHashForBinaryMessage(data)];
}
...
+ (XPYKey*) verifySignature:(NSData*)signature forBinaryMessage:(NSData *)data
{
    ...
    return [self verifyCompactSignature:signature forHash:XPYSignatureHashForBinaryMessage(data)];
}
...
{
    XPYKey* key = [[self class] verifySignature:signature forBinaryMessage:data];
    return [key isEqual:self];
}
...
+ (BOOL) isCanonicalPublicKey:(NSData*)data error:(NSError**)errorOut
{
    ...
    if (length < 33)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: too short.", @"")}];
        return NO;
    }
    ...
    if (bytes[0] == 0x04)
    {
        ...
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: length of uncompressed key must be 65 bytes.", @"")}];
    ...
    else if (bytes[0] == 0x02 || bytes[0] == 0x03)
    {
        ...
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: length of compressed key must be 33 bytes.", @"")}];
        
        return NO;
    }
    
    // Unknown public key format.
    if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Unknown non-canonical public key.", @"")}];
...
{
    // See https://paycointalk.org/index.php?topic=8392.msg127623#msg127623
    ...
    if (length < 9)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: too short.", @"")}];
        return NO;
    }
    ...
    if (length > 73)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: too long.", @"")}];
        return NO;
    }
    ...
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: unknown hashtype byte.", @"")}];
        return NO;
    }
    
    if (bytes[0] != 0x30)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: wrong type.", @"")}];
        return NO;
    }
    
    if (bytes[1] != length-3)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: wrong length marker.", @"")}];
        return NO;
    }
    ...
    if (5 + lenR >= length)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S length misplaced.", @"")}];
        return NO;
    }
    ...
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R+S length mismatch", @"")}];
        return NO;
    }
    ...
    if (R[-2] != 0x02)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value type mismatch", @"")}];
        return NO;
    }
    if (lenR == 0)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R length is zero", @"")}];
        return NO;
    }
    
    if (R[0] & 0x80)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value negative", @"")}];
        return NO;
    }
    
    if (lenR > 1 && (R[0] == 0x00) && !(R[1] & 0x80))
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value excessively padded", @"")}];
        return NO;
    }
    ...
    if (S[-2] != 0x02)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value type mismatch", @"")}];
        return NO;
    }
    
    if (lenS == 0)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S length is zero", @"")}];
        return NO;
    }
    
    if (S[0] & 0x80)
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value is negative", @"")}];
        return NO;
    }
    
    if (lenS > 1 && (S[0] == 0x00) && !(S[1] & 0x80))
    {
        if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain code:XPYErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value excessively padded", @"")}];
        return NO;
    }
    
    if (verifyLowerS)
    {
        if (!XPYKeyCheckSignatureElement(S, lenS, YES))
        {
            if (errorOut) *errorOut = [NSError errorWithDomain:XPYErrorDomain
                                                          code:XPYErrorNonCanonicalScriptSignature
...
static const unsigned char XPYKeyMaxModOrder[32] = {
...
static const unsigned char XPYKeyMaxModHalfOrder[32] = {
...
static const unsigned char XPYKeyZero[0] = {};

NSComparisonResult XPYKeyCompareBigEndian(const unsigned char *c1, size_t c1len,
...
static BOOL XPYKeyCheckPrivateKeyRange(const unsigned char *secret, size_t length)
{
    return XPYKeyCompareBigEndian(secret, length, XPYKeyZero, 0) > 0 &&
           XPYKeyCompareBigEndian(secret, length, XPYKeyMaxModOrder, 32) <= 0;
}

static BOOL XPYKeyCheckSignatureElement(const unsigned char *bytes, int length, BOOL half)
{
    return XPYKeyCompareBigEndian(bytes, length, XPYKeyZero, 0) > 0 &&
           XPYKeyCompareBigEndian(bytes, length, half ? XPYKeyMaxModHalfOrder : XPYKeyMaxModOrder, 32) <= 0;
}

static NSData* XPYSignatureHashForBinaryMessage(NSData* msg)
{
    NSMutableData* data = [NSMutableData data];
    [data appendData:[XPYProtocolSerialization dataForVarString:[@"Paycoin Signed Message:\n" dataUsingEncoding:NSASCIIStringEncoding]]];
    [data appendData:[XPYProtocolSerialization dataForVarString:msg ?: [NSData data]]];
    return XPYHash256(data);
}
