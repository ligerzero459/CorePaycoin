...
+ (XPYScriptChunk*) parseChunkFromData:(NSData*)data offset:(NSUInteger)offset;
...
- (id) initWithData:(NSData*)data
{
    if (self = [super init])
    {
        // It's important to keep around original data to correctly identify the size of the script for XPY_MAX_SCRIPT_SIZE check
        // and to correctly calculate hash for the signature because in PaycoinQT scripts are not re-serialized/canonicalized.
...
- (id) initWithAddress:(XPYAddress*)address
{
    ...
    if ([address isKindOfClass:[XPYPublicKeyAddress class]])
    {
        ...
        XPYOpcode prefix[] = {OP_DUP, OP_HASH160};
        ...
        XPYOpcode suffix[] = {OP_EQUALVERIFY, OP_CHECKSIG};
        [resultData appendBytes:suffix length:sizeof(suffix)];
        
        return [self initWithData:resultData];
    }
    else if ([address isKindOfClass:[XPYScriptHashAddress class]])
    {
        // OP_HASH160 <hash> OP_EQUAL
        NSMutableData* resultData = [NSMutableData data];
        
        XPYOpcode prefix[] = {OP_HASH160};
        [resultData appendBytes:prefix length:sizeof(prefix)];
...
- (id) initWithPublicKeys:(NSArray*)publicKeys signaturesRequired:(NSUInteger)signaturesRequired
{
    ...
    XPYOpcode m_opcode = XPYOpcodeForSmallInteger(signaturesRequired);
    XPYOpcode n_opcode = XPYOpcodeForSmallInteger(publicKeys.count);
    ...
    {
        NSData* d = [XPYScriptChunk scriptDataForPushdata:pubkey preferredLengthEncoding:-1];
    ...
    XPYOpcode checkmultisig_opcode = OP_CHECKMULTISIG;
...
- (NSData*) data
{
    if (!_data)
    {
        ...
        for (XPYScriptChunk* chunk in _chunks)
        {
...
- (NSMutableArray*) parseData:(NSData*)data
{
    ...
    while (i < length)
    {
        XPYScriptChunk* chunk = [XPYScriptChunk parseChunkFromData:data offset:i];
...
- (NSMutableArray*) parseString:(NSString*)string
{
    ...
    while (![scanner isAtEnd])
    {
        ...
        // The text could be:
        // 1. Single-quoted UTF8 string. (PaycoinQT unit tests style)
        // 2. Arbitrary pushdata in square brackets (paycoinj style)
        // 3. Raw binary in hex (starts with 0x) (PaycoinQT unit tests style)
        ...
        // 2. Arbitrary hex pushdata in square brackets (paycoinj style)
        else if ([scanner scanString:@"[" intoString:NULL])
        {
            ...
            if ([scanner scanUpToString:@"]" intoString:&hexstring])
            {
                ...
                NSData* data = XPYDataFromHex(hexstring);
        ...
        else if ([scanner scanUpToCharactersFromSet:whitespaceSet intoString:&word])
        {
            ...
            if (!mustBePushdata)
            {
                XPYOpcode opcode = XPYOpcodeForName(word);
                
                // Opcode may be used without OP_ prefix.
                if (opcode == OP_INVALIDOPCODE) opcode = XPYOpcodeForName([@"OP_" stringByAppendingString:word]);
                ...
                else
                {
                    ...
                    if ([[NSString stringWithFormat:@"%lld", decimalInteger] isEqualToString:word])
                    {
                        ...
                        {
                            opcode = XPYOpcodeForSmallInteger((NSUInteger)decimalInteger);
                            
                            if (opcode == OP_INVALIDOPCODE)
                            {
                                @throw [NSException exceptionWithName:@"XPYScript parser inconsistency"
                        ...
                        {
                            XPYBigNumber* bn = [[XPYBigNumber alloc] initWithInt64:decimalInteger];
                            [scriptData appendData:[XPYScriptChunk scriptDataForPushdata:bn.signedLittleEndian preferredLengthEncoding:-1]];
                            continue;
                        }
                    }
            ...
            NSData* data = XPYDataFromHex(word);
        ...
        if (pushdata)
        {
            NSData* addedScriptData = [XPYScriptChunk scriptDataForPushdata:pushdata preferredLengthEncoding:lengthEncoding];
...
- (BOOL) isPayToPublicKeyHashScript
{
    ...
    XPYScriptChunk* dataChunk = [self chunkAtIndex:2];
...
- (BOOL) isPayToScriptHashScript
{
    ...
    XPYScriptChunk* dataChunk = [self chunkAtIndex:1];
...
// Not used in CorePaycoin. Similar code is used in paycoin-ruby. I don't know if we'll ever need it.
...
- (void) detectMultisigScript
{
    ...
    XPYOpcode m_opcode = [self opcodeAtIndex:0];
    XPYOpcode n_opcode = [self opcodeAtIndex:-2];
    
    NSInteger m = XPYSmallIntegerFromOpcode(m_opcode);
    NSInteger n = XPYSmallIntegerFromOpcode(n_opcode);
...
- (void) enumerateOperations:(void(^)(NSUInteger opIndex, XPYOpcode opcode, NSData* pushdata, BOOL* stop))block
{
    if (!block) return;
...
- (XPYAddress*) standardAddress
{
    if ([self isPayToPublicKeyHashScript])
    {
        ...
        {
            return [XPYPublicKeyAddress addressWithData:dataChunk.pushdata];
        }
    }
    else if ([self isPayToScriptHashScript])
    {
        ...
        if (!dataChunk.isOpcode && dataChunk.range.length == 21)
        {
            return [XPYScriptHashAddress addressWithData:dataChunk.pushdata];
        }
    }
    return nil;
}
...
- (XPYScript*) scriptHashScript
{
    return [[XPYScript alloc] initWithAddress:[self scriptHashAddress]];
}

// Returns XPYScriptHashAddress that hashes this script.
// Equivalent to [[script scriptHashScript] standardAddress] or [XPYScriptHashAddress addressWithData:XPYHash160(script.data)]
- (XPYScriptHashAddress*) scriptHashAddress
{
    return [XPYScriptHashAddress addressWithData:XPYHash160(self.data)];
}

- (XPYScriptHashAddressTestnet*) scriptHashAddressTestnet
{
    return [XPYScriptHashAddressTestnet addressWithData:XPYHash160(self.data)];
}
...
- (XPYScript*) simulatedSignatureScriptWithOptions:(XPYScriptSimulationOptions)opts
{
    if ([self isPayToPublicKeyHashScript])
    {
        XPYScript* script = [XPYScript new];
        [script appendData:[XPYScript simulatedSignatureWithHashType:SIGHASH_ALL]];
        [script appendData:(opts & XPYScriptSimulationCompressedPublicKeys) ? [XPYScript simulatedCompressedPubkey] : [XPYScript simulatedUncompressedPubkey]];
        return script;
    }
    else if ([self isPublicKeyScript])
    {
        return [[XPYScript new] appendData:[XPYScript simulatedSignatureWithHashType:SIGHASH_ALL]];
    }
    else if ([self isPayToScriptHashScript])
    {
        // Check if allowed to simulate input for P2SH as 2-of-3 multisig.
        if (opts & XPYScriptSimulationMultisigP2SH)
        {
            ...
            [script appendData:[XPYScript simulatedSignatureWithHashType:SIGHASH_ALL]];
            [script appendData:[XPYScript simulatedSignatureWithHashType:SIGHASH_ALL]];
            [script appendData:[XPYScript simulatedMultisigScriptWithSignaturesCount:2 pubkeysCount:3
                                          compressedPubkeys:(opts & XPYScriptSimulationCompressedPublicKeys)].data];
            return script;
        }
        else
        {
...
+ (NSData*) simulatedSignatureWithHashType:(XPYSignatureHashType)hashtype
{
...
+ (XPYScript*) simulatedMultisigScriptWithSignaturesCount:(NSInteger)m pubkeysCount:(NSInteger)n compressedPubkeys:(BOOL)compressedPubkeys
{
    /*
     Script.new <<
     ...
     OP_CHECKMULTISIG
     */
    XPYScript* multisig = [XPYScript new];
    [multisig appendOpcode:XPYOpcodeForSmallInteger(m)];
    NSData* pubkey = compressedPubkeys ? [XPYScript simulatedCompressedPubkey] : [XPYScript simulatedUncompressedPubkey];
    for (NSInteger i = 0; i < n; i++)
    {
        [multisig appendData:pubkey];
    }
    [multisig appendOpcode:XPYOpcodeForSmallInteger(n)];
    return multisig;
}
...
- (XPYScript*) appendOpcode:(XPYOpcode)opcode
{
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    [scriptData appendBytes:&opcode length:sizeof(opcode)];
    
    XPYScriptChunk* chunk = [[XPYScriptChunk alloc] init];
    ...
    for (XPYScriptChunk* chunk in _chunks) chunk.scriptData = scriptData;
...
- (XPYScript*) appendData:(NSData*)data
{
    if (data.length == 0) return self;
    
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    NSData* addedScriptData = [XPYScriptChunk scriptDataForPushdata:data preferredLengthEncoding:-1];
    [scriptData appendData:addedScriptData];
...
- (XPYScript*) appendScript:(XPYScript*)otherScript
{
    ...
    for (XPYScriptChunk* chunk in otherScript->_chunks)
    {
        XPYScriptChunk* chunk2 = [[XPYScriptChunk alloc] init];
...
- (XPYScript*) deleteOccurrencesOfData:(NSData*)data
{
    if (data.length == 0) return self;
...
- (XPYScript*) deleteOccurrencesOfOpcode:(XPYOpcode)opcode
{
    NSMutableData* md = [NSMutableData data];
...
- (XPYScript*) subScriptFromIndex:(NSUInteger)index
{
    NSMutableData* md = [NSMutableData data];
    for (XPYScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(index, _chunks.count - index)])
    {
        [md appendData:chunk.chunkData];
    }
    return [[XPYScript alloc] initWithData:md];
}

- (XPYScript*) subScriptToIndex:(NSUInteger)index
{
    NSMutableData* md = [NSMutableData data];
    for (XPYScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(0, index)])
    {
        [md appendData:chunk.chunkData];
    }
    return [[XPYScript alloc] initWithData:md];
...
{
    return [[XPYScript alloc] initWithData:self.data];
}
...
- (XPYScriptChunk*) chunkAtIndex:(NSInteger)index
{
    XPYScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
    return chunk;
}
...
- (XPYOpcode) opcodeAtIndex:(NSInteger)index
{
    XPYScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
...
- (XPYBigNumber*) bignumberAtIndex:(NSInteger)index
{
    NSData* data = [self pushdataAtIndex:index];
    if (!data) return nil;
    XPYBigNumber* bn = [[XPYBigNumber alloc] initWithSignedLittleEndian:data];
    return bn;
}
...
#pragma mark - Canonical checks (Deprecated methods - use XPYKey API)


+ (BOOL) isCanonicalPublicKey:(NSData*)data error:(NSError**)errorOut
{
    return [XPYKey isCanonicalPublicKey:data error:errorOut];
}

+ (BOOL) isCanonicalSignature:(NSData*)data verifyEvenS:(BOOL)verifyEvenS error:(NSError**)errorOut
{
    return [XPYKey isCanonicalSignatureWithHashType:data verifyEvenS:verifyEvenS error:errorOut];
}
...
@implementation XPYScriptChunk {
}

- (XPYOpcode) opcode
{
    return (XPYOpcode)((const unsigned char*)_scriptData.bytes)[_range.location];
}

- (BOOL) isOpcode
{
    XPYOpcode opcode = [self opcode];
...
// Some other guys (PaycoinQT, paycoin-ruby) encode "small enough" integers in decimal numbers and do that differently.
// PaycoinQT encodes any data less than 4 bytes as a decimal number.
// paycoin-ruby encodes 2..16 as decimals, 0 and -1 as opcode names and the rest is in hex.
...
- (NSString*) string
{
    ...
    if (self.isOpcode)
    {
        ...
        {
            return XPYNameForOpcode(opcode);
        }
    }
    else
    {
...
+ (XPYScriptChunk*) parseChunkFromData:(NSData*)scriptData offset:(NSUInteger)offset
{
