#import "XPYCurrencyConverter.h"
#import "XPYNumberFormatter.h"

@implementation XPYCurrencyConverter

- (id) init
{
    if (self = [super init])
    {
        _mode = XPYCurrencyConverterModeAverage;
    }
    return self;
}
...
{
    switch (_mode) {
        case XPYCurrencyConverterModeAverage:
            if ([self isZero:_averageRate]) return 0;
            return XPYAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_averageRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case XPYCurrencyConverterModeBuy:
            if ([self isZero:_buyRate]) return 0;
            return XPYAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_buyRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case XPYCurrencyConverterModeSell:
            if ([self isZero:_sellRate]) return 0;
            return XPYAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_sellRate] decimalNumberByMultiplyingByPowerOf10:8]);

            // TODO: add order book modes

        default:
            [[NSException exceptionWithName:@"XPYCurrencyConverter Not supported Mode" reason:@"This mode is not supported yet" userInfo:nil] raise];
            break;
    }
...
{
    return [[XPYCurrencyConverter alloc] initWithDictionary:self.dictionary];
}
