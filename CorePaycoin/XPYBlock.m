...
@interface XPYBlock ()
@property(nonatomic, readwrite) XPYBlockHeader* header;
@end

@implementation XPYBlock

- (id) init
{
    if (self = [super init]) {
        self.header = [[XPYBlockHeader alloc] init];
    }
    return self;
}

- (id) initWithHeader:(XPYBlockHeader*)header
{
    if (self = [super init]) {
        self.header = header;
    }
...
{
    XPYBlock* b = [[XPYBlock alloc] init];
