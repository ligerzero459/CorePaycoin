...
#import "XPYErrors.h"
#import "XPYFancyEncryptedMessage.h"
#import "XPYData.h"
#import "XPYKey.h"
...
#define XPYFancyEncryptedMessageChecksumLength 16

static uint8_t XPYEMCompactTargetForFullTarget(uint32_t fullTarget);
static uint32_t XPYEMFullTargetForCompactTarget(uint8_t compactTarget);

@implementation XPYFancyEncryptedMessage {
...
- (id) initWithData:(NSData*)data
{
    ...
    if (self = [super init])
    {
        ...
        _addressLength = XPYFancyEncryptedMessageAddressLengthNone;
        _decryptedData = data;
    }
    return self;
}

- (NSData*) encryptedDataWithKey:(XPYKey*)recipientKey seed:(NSData*)seed0
{
    ...
    self.address = XPYHash160(recipientPubKey);
    ...
    else
    {
        ...
        void* randomBytes = XPYCreateRandomBytesOfLength(32);
        CC_SHA256_Update(&ctx256, randomBytes, 32);
        XPYSecureMemset(randomBytes, 0, 32);
        free(randomBytes);
    }
    ...
    XPYSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
    XPYDataClear(recipientPubKey);
    
    if (self.timestampVariance > 0)
    {
        NSData* varianceHash = XPYSHA256(seed); // extra hashing to not leak our seed that we'll use for private key nonce.
        uint32_t rand = *((uint32_t*)varianceHash.bytes);
        self.timestamp -= (rand % self.timestampVariance);
    }
    
    uint8_t compactTarget = XPYEMCompactTargetForFullTarget(_difficultyTarget);
    ...
    do {
        ...
        XPYDataClear(messageData); // clear previous trial
        
        [messageData setLength:0];
        
        [messageData appendBytes:XPYFancyEncryptedMessageVersion length:4];
        ...
        XPYKey* nonceKey = [[XPYKey alloc] initWithPrivateKey:privkey];
        ...
        XPYBigNumber* pk = [[XPYBigNumber alloc] initWithUnsignedBigEndian:privkey];
        
        XPYCurvePoint* curvePoint = recipientKey.curvePoint;
        ...
        {
            XPYDataClear(encryptedData);
            XPYSecureMemset(&ctx256, 0, sizeof(ctx256));
            encryptedData = nil;
        }
        else
        {
        ...
        {
            XPYDataClear((NSMutableData*)pointX);
        }
        [pk clear];
        [curvePoint clear];
        XPYSecureMemset(&ctx256, 0, sizeof(ctx256));
        ...
        [messageData appendData:[XPYProtocolSerialization dataForVarString:encryptedData]];
        ...
        [messageData appendBytes:digest256 length:XPYFancyEncryptedMessageChecksumLength];
...
- (id) initWithEncryptedData:(NSData*)data
{
    if (self = [super init])
    {
        ...
        if (datalength < (4 + 1 + 1 + 4 + 1 + 1 + 32 + 1 + XPYFancyEncryptedMessageChecksumLength)) return nil;
        ...
        if (memcmp(XPYFancyEncryptedMessageVersion, msgbytes, sizeof(XPYFancyEncryptedMessageVersion)) != 0)
        {
            return nil;
        }
        ...
        uint32_t target = XPYEMFullTargetForCompactTarget(msgbytes[4]);
        ...
        XPYKey* nonceKey = [[XPYKey alloc] initWithPublicKey:pubkeyNonceData];
        ...
        _encryptedData = [XPYProtocolSerialization readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, datalength - offset)] readBytes:&encodedMessageLength];
        
        if (_encryptedData == nil) return nil;
        
        offset += encodedMessageLength;
        
        if (datalength < offset + XPYFancyEncryptedMessageChecksumLength) return nil;
        
        _checksum = [data subdataWithRange:NSMakeRange(offset, XPYFancyEncryptedMessageChecksumLength)];
...
- (NSData*) decryptedDataWithKey:(XPYKey*)key error:(NSError**)errorOut
{
    ...
    XPYBigNumber* pk = [[XPYBigNumber alloc] initWithUnsignedBigEndian:key.privateKey];
    
    XPYCurvePoint* curvePoint = _nonceKey.curvePoint;
    ...
    if (_checksum.length != XPYFancyEncryptedMessageChecksumLength)
    {
        XPYSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
    
    if (memcmp(digest256, _checksum.bytes, XPYFancyEncryptedMessageChecksumLength) != 0)
    {
        XPYSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
...
{
    return XPYEMCompactTargetForFullTarget(target);
}
...
{
    return XPYEMFullTargetForCompactTarget(compactTarget);
}
...
static uint8_t XPYEMCompactTargetForFullTarget(uint32_t fullTarget)
{
    ...
    {
        if (XPYEMFullTargetForCompactTarget(ct) <= fullTarget)
        {
            uint32_t order = ct >> 3;
