...
#import "XPYMnemonic.h"
#import "XPYData.h"
#import "XPYKeychain.h"
#import "XPYProtocolSerialization.h"
#include <CommonCrypto/CommonKeyDerivation.h>

@interface XPYMnemonic ()

@property(nonatomic, readwrite) XPYMnemonicWordListType wordListType;
...
@property(nonatomic, readwrite) XPYKeychain* keychain;

@end

static inline BOOL XPYMnemonicIsBitSet(uint8_t* buf, int bitIndex)
{
...
static inline void XPYMnemonicSetBit(uint8_t* buf, int bitIndex)
{
...
static inline void XPYMnemonicIntegerTo11Bits(uint8_t* buf, int bitIndex, int integer)
{
    for (int i = 0; i < 11; i++) {
        if ((integer & 0x400) == 0x400)
        {
            XPYMnemonicSetBit(buf, bitIndex + i);
        }
        integer = integer << 1;
    }
}

static inline NSUInteger XPYMnemonicIntegerFrom11Bits(uint8_t* buf, int bitIndex)
{
    NSUInteger value = 0;
    for (int i = 0; i < 11; i++)
    {
        if (XPYMnemonicIsBitSet(buf, bitIndex + i)) {
            value = (value << 1) | 0x01;
        } else {
            value = (value << 1);
        }
...
@implementation XPYMnemonic


- (id) initWithEntropy:(NSData*)entropy password:(NSString*)password wordListType:(XPYMnemonicWordListType)wordListType
{
    if (!entropy) return nil;
    if (wordListType == XPYMnemonicWordListTypeUnknown) return nil;
...
- (id) initWithWords:(NSArray*)words password:(NSString*)password wordListType:(XPYMnemonicWordListType)wordListType
{
    if (!words) return nil;
    ...
    if (self = [super init])
    {
        ...
        if (wordListType != XPYMnemonicWordListTypeUnknown)
        {
...
- (id) initWithData:(NSData*)data
{
    ...
    if (self = [super init])
    {
        NSAssert(sizeof(XPYMnemonicWordListType) == 1, @"must be 1 byte long");

        if (data.length < sizeof(XPYMnemonicWordListType))
        {
            // Not enough bytes.
            return nil;
        }
        ...
        _wordListType = ((XPYMnemonicWordListType*)data.bytes)[offset];

        offset += sizeof(XPYMnemonicWordListType);

        if (_wordListType != XPYMnemonicWordListTypeEnglish)
        {
            // Not supported list.
            return nil;
        }
        ...
        _entropy = [XPYProtocolSerialization
        ...
        NSData* passwordData = [XPYProtocolSerialization
                                readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
        ...
        if (!passwordData)
        {
            ...
            NSLog(@"ERROR: XPYMnemonic cannot decode password (if missing, password should be empty string with 0x00 length prefix).");
            return nil;
        }
        ...
        if (!_password)
        {
            ...
            NSLog(@"ERROR: XPYMnemonic cannot convert password back to UTF8 string.");
            return nil;
        }
        ...
        {
            _seed = [XPYProtocolSerialization
                     readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
                     readBytes:NULL];

            if (!_seed)
            {
                NSLog(@"ERROR: XPYMnemonic failed to read encoded seed from mnemonic payload. Will recompute it lazily when needed.");
            }
        }
    }
    return self;
...
- (XPYKeychain*) keychain
{
    if (!_keychain)
    {
        _keychain = [[XPYKeychain alloc] initWithSeed:self.seed];
    }
    return _keychain;
}
...
- (NSData*) dataWithSeed:(BOOL)withSeed
{
    ...
    if (_wordListType == XPYMnemonicWordListTypeUnknown) return nil;

    [data appendBytes:&_wordListType length:sizeof(_wordListType)];

    if (!_entropy) return nil;

    [data appendData:[XPYProtocolSerialization dataForVarString:_entropy]];

    NSData* passwordData = [_password ?: @"" dataUsingEncoding:NSUTF8StringEncoding];

    [data appendData:[XPYProtocolSerialization dataForVarString:passwordData]];

    if (withSeed)
    {
        NSData* seed = self.seed;

        [data appendData:[XPYProtocolSerialization dataForVarString:seed]];
    }
...
- (BOOL) isEqual:(XPYMnemonic*)other
{
...
- (NSArray*) wordsForEntropy:(NSData*)entropy wordListType:(XPYMnemonicWordListType)listType
{
    if (!entropy) return nil;
    ...
    NSData* checksumHash = XPYSHA256(entropy);
    ...
    {
        NSUInteger wordIndex = XPYMnemonicIntegerFrom11Bits((uint8_t*)buf.bytes, i * 11);
...
- (NSData*) entropyFromWords:(NSArray*)words wordListType:(XPYMnemonicWordListType)listType
{
    if (listType == XPYMnemonicWordListTypeUnknown) return nil;
    if (!words) return nil;
    ...
    for (int i = 0; i < words.count; i++)
    {
        ...
        XPYMnemonicIntegerTo11Bits((uint8_t*)buf.mutableBytes, i * 11, (int)wordIndex);
    }
...
+ (NSArray*) wordListForType:(XPYMnemonicWordListType)type
{
    if (type == XPYMnemonicWordListTypeEnglish)
    {
