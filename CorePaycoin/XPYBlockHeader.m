...
- (id) init
{
    ...
    {
        // init default values
        _version = XPYBlockCurrentVersion;
        _previousBlockHash = XPYZero256();
        _merkleRootHash = XPYZero256();
        _time = 0;
        _difficultyTarget = 0;
        _nonce = 0;
...
{
    return XPYIDFromHash(self.previousBlockHash);
}

- (void) setPreviousBlockID:(NSString *)previousBlockID
{
    self.previousBlockHash = XPYHashFromID(previousBlockID) ?: XPYZero256();
}
...
- (NSData*) computePayload
{
    ...
    [data appendData:self.previousBlockHash ?: XPYZero256()];
    [data appendData:self.merkleRootHash ?: XPYZero256()];
...
{
    XPYBlockHeader* bh = [[XPYBlockHeader alloc] init];
